		...
		<div className="mb-10 mx-10 p-5 flex justify-between rounded-md bg-white items-start gap-7">
				...
				<div className="mt-3 flex flex-col gap-4">
					{items.length ? (
						items.map((url: string, idx: number) => (
							<div
								className={`flex justify-between items-center p-2 ${idx % 2 === 0 ? 'bg-gray-400' : 'bg-gray-100'}`}
			...
			<div className="text-black text-xl w-[800px]">
				<div className="bg-gray-300 rounded-md p-4 gap-3 flex flex-col">
					<div className="text-white bg-gray-500 p-2 rounded font-sans font-bold text-left text-2xl">Screenshot:</div>
					<Image
						alt="Screenshot"
						className="rounded"
						height={90 * 5}
